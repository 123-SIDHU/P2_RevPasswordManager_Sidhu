...
import com.rev.app.controller.VaultController;
...
class VaultControllerTest {
    ...
    void testFavorites() {
        User user = User.builder().id(1L).build();
        when(authUtil.getCurrentUser()).thenReturn(user);
        when(vaultService.getFavorites(1L)).thenReturn(new ArrayList<>());

        assertEquals("vault/favorites", controller.favorites(model));
    }
